...
echo "\033[1;32m╔═══════════════════════════════════════════════════════════╗\n";
usleep(70000);
echo $thuong.$vang."Tool Tương Tác Chéo Cookie Đa Luồng\n";
usleep(70000);
...
$dem = 0;
//token ttc
$_SESSION['check'] = file_exists("logtoolttc_cookie.txt");
if ($_SESSION['check'] =='1'){
  luachon:
...
$my = fopen("logtoolttc_cookie.txt", "w+");
...
fwrite($my,json_encode($arr));
    $my = file("logtoolttc_cookie.txt");
$bb = file_get_contents('logtoolttc_cookie.txt');
$cc =json_decode($bb);
...
array_push($khocookie,$nhapck);
            ...
            $k = fopen("cookiefb_toolttc.txt","a+");
fwrite($k, $js);
fclose($k);
echo $thuong.$luc."Bạn đã nhập ".$vang.$demcki." ".$luc."Cookie Facebook\n";
sleep(2);
    }else{
        $khocookie = json_decode(fread(fopen("cookiefb_toolttc.txt","r"),filesize("cookiefb_toolttc.txt")),true);
        $demcki = count($khocookie);
    }
@system('clear');
echo "\033[1;32m╔═══════════════════════════════════════════════════════════╗\n";
...
echo $luc."╚═══════════════════════════════════════════════════════════╝\n";
usleep(70000); 
echo $thuong.$red."Tài Khoản TTC: ".$vang.$user."\n";
echo $thuong.$trang."Số FB Đang Chạy:$yellow $demcki Nick\n";
$xu = getxu();
echo $thuong.$cyan."Xu Hiện Tại:$yellow $xu\n";
echo $thanhngang;
echo $thuong.$luc."Nhập ".$do."[".$vang."1".$do."]".$luc." Nhiệm Vụ Like\n"; 
echo $thuong.$luc."Nhập ".$do."[".$vang."2".$do."]".$luc." Nhiệm Vụ Comment\n"; 
echo $thuong.$luc."Nhập ".$do."[".$vang."3".$do."]".$luc." Nhiệm Vụ Share\n"; 
echo $thuong.$luc."Nhập ".$do."[".$vang."4".$do."]".$luc." Nhiệm Vụ Cảm Xúc\n"; 
echo $thuong.$luc."Nhập ".$do."[".$vang."5".$do."]".$luc." Nhiệm Vụ Cảm Xúc CMT\n"; 
echo $thanhngang;
...
if ($access_token == 'die') {
    echo "\r";
    echo "                                                      \r";
    echo $thuong.$do."Cookie Facebook Bị Die!!!\n";
...
break;
                    ...
                    if (strpos($nhiemvu, '5') !== false) {
                        ...
                        if (count($listlikecmt) == 0) {
                            $rd = rand(1, 7);
                            echo $thuong.$rd.$red." Đang tạm thời hết nhiệm vụ cx cmt       \r";
                        }
                    }
...
echo "\r";
       echo "                                                      \r";
        ...
        function hoanthanh($dem, $type, $id, $xujob, $xu) {
            ...
            echo "\r";
            echo "                                              \r";
            $a = "\033[1;33m[".$dem."\033[1;33m] \033[1;31m● \033[1;36m".$tim.date("H:i:s")."\033[1;31m ●\033[1;3".$rd."m$type\033[1;31m● \033[1;97m Hoàn Thành \e[0m\033[1;31m● \033[1;32m+$xujob \033[1;31m● \033[1;93m".$xu." \033[1;91mXu\n";
            $len = strlen($a);
            for ($x = 0;$x < $len;$x++) {
                echo $a[$x];
            }
